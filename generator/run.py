...
if args.analysis_type == "retrieval_recall":
    ...
    cmds = []
    if args.batch is True and 'gpt' in args.model:  # use batch, run simo
        for ret_acc in ret_acc_list:
            cmd = f'python generator/generate.py --model {args.model} --temperature {args.temperature} --batch --dataset {args.dataset} --retriever {args.retriever} --analysis_type {args.analysis_type} --n {args.n} --ret_acc {ret_acc}'
            cmds.append(cmd)
        batch_cmd = ''
        for cmd in cmds:
    ...
    if args.batch is True and 'gpt' in args.model:  # use batch, run simo
        for ret_doc_type in ret_doc_type_list:
            cmd = f'python generator/generate.py --model {args.model} --temperature {args.temperature} --batch --dataset {args.dataset} --retriever {args.retriever} --analysis_type {args.analysis_type} --n {args.n} --ret_doc_type {ret_doc_type}'
            cmds.append(cmd)
        batch_cmd = ''
        for cmd in cmds:
            batch_cmd = batch_cmd + cmd + ' & '
    ...
    if args.dataset in ['NQ', 'TriviaQA', 'hotpotQA']: doc_selection_type_list = ['top_1', 'top_5', 'top_10', 'top_15', 'top_20', 'top_25', 'top_30', 'top_35', 'top_40']
    else: doc_selection_type_list = ['top_1', 'top_3', 'top_5', 'top_7', 'top_9', 'top_11', 'top_13', 'top_15']
    cmds = []
    if args.batch is True and 'gpt' in args.model:  # use batch, run simo
        for doc_selection_type in doc_selection_type_list:
            cmd = f'python generator/generate.py --model {args.model} --temperature {args.temperature} --batch --dataset {args.dataset} --retriever {args.retriever} --analysis_type {args.analysis_type} --n {args.n} --doc_selection_type {doc_selection_type}'
            cmds.append(cmd)
        batch_cmd = ''
        for cmd in cmds:
            batch_cmd = batch_cmd + cmd + ' & '
    ...
    if args.batch is True and 'gpt' in args.model:  # use batch, run simo
        for pl_analysis in pl_analysis_list:
            cmd = f'python generator/generate.py --model {args.model} --temperature {args.temperature} --batch --dataset {args.dataset} --retriever {args.retriever} --analysis_type {args.analysis_type} --n {args.n} --pl_analysis {pl_analysis}'
            cmds.append(cmd)
        batch_cmd = ''
        for cmd in cmds:
            batch_cmd = batch_cmd + cmd + ' & '
        ...
        for pl_analysis in pl_analysis_list:
            cmd = f'python generator/generate.py --model {args.model} --temperature {args.temperature} --dataset {args.dataset} --retriever {args.retriever} --analysis_type {args.analysis_type} --n {args.n} --pl_analysis {pl_analysis}'
            subprocess.check_output(cmd, shell=True)
