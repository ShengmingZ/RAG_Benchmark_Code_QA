...
class Generator:
    ...
    def gene_prompts(self):
        if self.analysis_type == 'retrieval_doc_selection' and 'pl' in self.doc_selection_type:
            ...
            if 'type2' in self.pl_analysis:
                ret_doc_keys_list, prompts, pl_list = gene_prompts_for_pl_analysis_type2(pl_analysis=self.pl_analysis,
...
if __name__ == '__main__':
    ...
    in_program_call = '--model gpt-3.5-turbo-0125 --temperature 0 --n 1 --dataset pandas_numpy_eval --retriever openai-embedding --analysis_type prompt_method --prompt_type ir-cot --action gene_responses'
    args = generate_config(in_program_call)
    ...
    if args.action == 'gene_prompts':
        ...
        pred_eval(args, if_eval_retrieval=False, if_code_analysis=True, if_calc_perplexity=True, if_save=True)
